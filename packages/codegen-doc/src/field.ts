...
export function isScalarField(context: PluginContext, field: FieldDefinitionNode): boolean {
  const typeName = reduceTypeName(field.type);
  // Check if it's in scalars map OR if it's a built-in GraphQL scalar
  const builtInScalars = ['String', 'Int', 'Float', 'Boolean', 'ID'];
  return Object.keys(context.scalars).includes(typeName) || builtInScalars.includes(typeName);
}
