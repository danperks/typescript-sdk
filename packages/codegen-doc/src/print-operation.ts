...
export function printOperations(
  ...
  if (depth > 5) {
    logger.warn(log, `Maximum nesting depth (5) reached for field chain: ${fields.map(f => f.name.value).join(' -> ')}`);
    return undefined;
  }

  if (isValidField(context, lastField)) {
    // Check if this field has required arguments that we can't provide
    const hasRequiredArgs = lastField.arguments?.some(arg => arg.type.kind === Kind.NON_NULL_TYPE);
    if (hasRequiredArgs && depth > 0) {
      // Skip nested operations that require arguments we can't provide
      return undefined;
    }
    ...
    if (type === OperationType.query) {
      ...
      const fieldOperations = object?.fields?.map(field => {
        if (
          ...
          /** Skip if field has required arguments at nested level */
          (depth > 0 && field.arguments?.some(arg => arg.type.kind === Kind.NON_NULL_TYPE))
        ) {
          return undefined;
        } else {
          /** For any objects create a new query for each nested field */
          return printOperations(context, type, [...fields, field], depth + 1);
        }
      });
